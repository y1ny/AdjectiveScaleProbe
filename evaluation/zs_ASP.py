...
# forked from 
# https://github.com/bigscience-workshop/t-zero/blob/master/evaluation/run_eval.py
...
import csv
#os.environ["CUDA_VISIBLE_DEVICES"] = '7,2,3,0'
...
task_to_keys = {
    "scale": ("premise", "hypothesis", "label"),
}
...
def main():
    ...
    args.template_name = template_list[0]
    args.model_name_or_path = "/path/to/model/T0pp"
    model_type = os.path.split(args.model_name_or_path)[-1]
    args.output_dir = '/path/to/debug'
    ...
    for test_dir in dir_list:
        ...
        scale_list = os.listdir(f"{test_path}/{test_dir}")
        for scale_file in scale_list:
            scale_path = f"{test_path}/{test_dir}/{scale_file}"
            
            # Downloading and loading a dataset from the hub.
            premise,hypo,labels = read_scale_data(scale_path)
            task_name = args.dataset_name
            raw_datasets = load_dataset('tsv2',
                            f"{scale_file[:-4]}",
                            data_files={"data": scale_path},
                            cache_dir=cache_dir,
                            split='data')
            

            in_need_line = [task_to_keys[task_name][0], task_to_keys[task_name][1], task_to_keys[task_name][2]]
            ...
            #     dataset_name='glue',subset_name='mnli'
            # )
            prompts = DatasetTemplates(
            ...
            for template_name in template_list:
                try:
                    ...
                    continue
                template_save_name = template_name.split(" ")[-2]
                save_path = f"/result/t0/{model_type.lower()}_{template_save_name}/{test_dir}"
                createDir(save_path)
                nli_class = ['entailment','neutral','contradiction']
                def preprocess_function(examples):
                    ...
                    for i in range(bs):
                        ...
                        ex['label'] = 'contradiction' if ex['label'] == 'not_entailment' else 'entailment'
                        input, target = template.apply(ex)
                        ex_answer_choices = template.get_answer_choices_list(ex)
                        if ex['label'] not in nli_class:
                            continue
                        target = ex_answer_choices[nli_class.index(ex['label'])]
                        input_texts.append(input)
                        target_texts.append(target)
                        answer_choices_texts.append(ex_answer_choices)
                    print(input_texts[0])
                    print(target_texts[0])
                    print(answer_choices_texts[0])
                ...
                eval_dataloader = DataLoader(eval_dataset, collate_fn=data_collator, shuffle=False, batch_size=args.per_device_eval_batch_size)
                ...
                logger.info(f"  test data path = {scale_path}")
                ...
                for batch in eval_dataloader:
                    ...
                    for i in range(min(total_batch_size,len(predictions))):
                        pred.append(nli_class[int(predictions[i])])
                ...
                with open(f'{save_path}/{scale_file}','w') as f:
                    collapse2two = {'entailment':'entailment',
                    'neutral':'not_entailment',
                    'contradiction':'not_entailment'}
                    for idx in range(len(premise)):
                        if labels[idx] != -2:
                            f.write(f'{premise[idx]}\t{hypo[idx]}\t{collapse2two[nli_class[int(labels[idx])]]}\t{collapse2two[pred[idx]]}\n')
                        else:
                            f.write(f'{premise[idx]}\t{hypo[idx]}\tdiscard\tdiscard\n')
