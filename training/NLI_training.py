...
import datasets
import numpy as np
from datasets import sload_dataset, load_metric
import re
import transformers
...
task_to_keys = {
    "multinli": ("sentence1", "sentence2", "label"),
}

dataset_filepath_list = {
    "multinli-o": "/path/to/MNLI",
}
...
def main():
    ...
    task_name = "multinli"
    model_name = f"/path/to/model/{model_args.model_name_or_path}"

    # add args

    training_args.output_dir = f"/path/to/model/NLI_model/{os.path.split(model_name)[1]}-{task_name}"
    ...
    # training_args.num_train_epochs = 2.0
    # training_args.save_steps = 5000
    # training_args.warmup_steps = 160
    # training_args.eval_steps=1000

    data_args.overwrite_cache = True
    data_args.task_name = task_name.lower()
    data_args.train_file = os.path.join(dataset_filepath_list[task_name], "train.tsv")
    data_args.validation_file = os.path.join(dataset_filepath_list[task_name], "dev.tsv")
